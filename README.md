...
Tabtint is a small plugin marketplace for tinting terminal tabs from local coding-agent state.

The first plugin, `tabtint-iterm2`, installs lifecycle hooks for Codex and Claude Code that set the current iTerm2 tab color:
...
The two agents use separate marketplace and plugin metadata, but they call the same bundled script.
...
```bash
jq empty .agents/plugins/marketplace.json .claude-plugin/marketplace.json plugins/tabtint-iterm2/.codex-plugin/plugin.json plugins/tabtint-iterm2/.claude-plugin/plugin.json plugins/tabtint-iterm2/hooks.json plugins/tabtint-iterm2/hooks/hooks.json
